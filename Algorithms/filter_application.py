#################################################################################
#                                                                               #
#               HFP filter design and application in audio file                 #
#     																			#
#################################################################################

"""
	This code demonstrates how to design a IIR filter with signal.iirfilter
	and apply to to an audio sample using the real_time_dsp lib. 
	The filter application is abstract and it will work with any order.
	Also shows how to plot a graph with fixed grid axis.
"""
...
######################################## Variables ######################################
# Checking variables
plot_filter=True #Plot  filter design results
wav_out=True #Output a wav file with filtered data
...
b, a = signal.iirfilter(order, fc, btype=b_type,analog=False,ftype=filter_type,output=output_type)
"""
            jw                 -jw              -jwM
